"""
Figure 3 (Nature-style rebuild) — representation learning / PLM embeddings.
...
# (ColorBrewer Dark2 brown). ponytail: hard-coded 10th hue, revisit if a class is added.
...
def scatter_pf(ax, XY, psr):
    # 0 = Fail (orange), 1 = Pass (blue). Plot Fail first so neither class is buried.
    for v, col in [(0, ok.FAIL), (1, ok.PASS)]:
        m = psr.values == v
