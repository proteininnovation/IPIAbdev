...
#   python tests/test_delphi.py --fast           # 5-fold CV, skip final-train (5)
...
# ══════════════════════════════════════════════════════════════════════════════
# TEST 4 — k-fold cross-validation (10-fold default, 5-fold in --fast)
# ══════════════════════════════════════════════════════════════════════════════
def test_kfold(fast: bool = False) -> bool:
    k = "5" if fast else "10"
    _head(f"── Test 4: {k}-fold cross-validation "
          f"{'(--fast)' if fast else '':<14}────────────────")
...
def main():
    ...
    if args.fast:    print("  Mode    : fast (5-fold CV, final-train skipped)")
    ...
    if _should(4):
        # --fast runs a lighter 5-fold CV (still real validation) instead of
        # skipping. 10-fold otherwise.
    ...
    labels = {
        ...
        4: f"{'5' if args.fast else '10'}-fold cross-validation  (4 models)",
