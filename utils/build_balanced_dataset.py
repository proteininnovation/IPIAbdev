...
Two strategies via --strategy:

  cluster  (default)
    Greedy Levenshtein clustering on CDR3 or HSEQ.
...
-----
  ...
  python build_balanced_dataset.py --input ipi_sec.xlsx --label sec_filter --min-total 6000
  python build_balanced_dataset.py --input ipi_sec.xlsx --label sec_filter --cluster-col HSEQ
  python build_balanced_dataset.py --input ipi_sec.xlsx --label sec_filter --strategy kmer_consensus
  python build_balanced_dataset.py --input ipi_sec.xlsx --label sec_filter --strategy kmer_consensus --kmer-col HSEQ --min-prob 0.7 --cv 3
"""
...
def build_balanced_dataset(
    ...
    seq_col = cluster_col if strategy == "cluster" else kmer_col
    sep = "=" * 62
    ...
    if seq_col not in df.columns:
        sys.exit("[ERROR] Column " + repr(seq_col) + " not found.")

    df = _clean_label(df, label_col)
    df = _clean_seq(df, seq_col)
    ...
    if strategy == "cluster":
        ...
        sys.exit("[ERROR] Unknown strategy " + repr(strategy) + ". Choose: cluster | kmer_consensus")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Two strategies: cluster (Levenshtein) or kmer_consensus (RF+XGBoost OOF). "
    ...
    parser.add_argument("--strategy",    default="cluster",
                        choices=["cluster", "kmer_consensus"],
                        help="Downsampling strategy (default: cluster)")
