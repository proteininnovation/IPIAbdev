...
# ============================================================
# STRATEGY 3 -- COMBINED (Cluster diversity + OOF confidence)
# ============================================================

def _combined_select_n(df_pool, n_target, label_str=""):
    """
    Select n_target rows from a pool that already has both
    _is_rep (bool) and _consensus (bool) and _mean_oof_prob (float).

    Tiered priority:
      Tier 1: rep=True  AND consensus=True   (diverse + confident)
      Tier 2: rep=True  AND consensus=False  (diverse but uncertain)
      Tier 3: rep=False AND consensus=True   (confident but redundant)
      Tier 4: rep=False AND consensus=False  (last resort)
    Within each tier ranked by _mean_oof_prob descending.
    """
    df = df_pool.copy()
    df["_tier"] = 4
    df.loc[ df["_is_rep"] &  df["_consensus"], "_tier"] = 1
    df.loc[ df["_is_rep"] & ~df["_consensus"], "_tier"] = 2
    df.loc[~df["_is_rep"] &  df["_consensus"], "_tier"] = 3

    df_ranked = df.sort_values(
        ["_tier", "_mean_oof_prob"], ascending=[True, False]
    ).reset_index(drop=True)

    # Print tier breakdown
    tag = "[combined " + label_str + "]" if label_str else "[combined]"
    for t, desc in [(1, "rep+consensus"), (2, "rep only"), (3, "consensus only"), (4, "fallback")]:
        n_t = int((df_ranked["_tier"] == t).sum())
        print("  " + tag + " tier " + str(t) + " (" + desc + ") :", f"{n_t:,}")
    print("  " + tag + " target :", f"{n_target:,}")

    selected = df_ranked.head(n_target)
    if len(selected) < n_target:
        print("  [WARN " + tag + "] Only", len(selected),
              "majority rows available (< target", n_target, ")")

    n_t1 = int((selected["_tier"] == 1).sum())
    n_t2 = int((selected["_tier"] == 2).sum())
    n_t3 = int((selected["_tier"] == 3).sum())
    n_t4 = int((selected["_tier"] == 4).sum())
    note = ("T1=" + str(n_t1) + " T2=" + str(n_t2)
            + " T3=" + str(n_t3) + " T4=" + str(n_t4)
            + " total=" + str(len(selected)))

    drop_cols = ["_is_rep", "_cluster", "_consensus",
                 "_rf_oof_prob", "_xgb_oof_prob", "_mean_oof_prob", "_tier"]
    return selected.drop(columns=drop_cols, errors="ignore").reset_index(drop=True), note


def downsample_combined(df_majority, df_full, label_col, cluster_col, threshold,
                        kmer_col, min_prob, n_balanced, n_imbalanced,
                        random_state, k_sizes=(1, 2, 3), cv=5):
    """
    Run clustering AND OOF scoring once each, merge into a single tiered pool,
    then slice at two sizes.

    Both expensive steps run exactly once regardless of how many output
    datasets are requested.

    Returns (selected_balanced, selected_imbalanced, strategy_note).
    """
    # Step 1: cluster majority sequences -> _is_rep, _cluster columns
    print("")
    print("[combined] Step 1/2: CDR3 clustering for diversity ...")
    df_pool = _cluster_build_pool(df_majority, cluster_col, threshold, random_state)

    # Step 2: OOF scoring -> _rf_oof_prob, _xgb_oof_prob, _mean_oof_prob columns
    print("")
    print("[combined] Step 2/2: GridSearchCV + OOF scoring for confidence ...")
    df_scored = _kmer_score_pool(
        df_majority, df_full, label_col, kmer_col, random_state, k_sizes, cv)

    # Merge: df_pool and df_scored are both reset-indexed copies of df_majority
    # so positional alignment is exact
    df_pool = df_pool.reset_index(drop=True)
    df_scored = df_scored.reset_index(drop=True)

    df_pool["_rf_oof_prob"]   = df_scored["_rf_oof_prob"]
    df_pool["_xgb_oof_prob"]  = df_scored["_xgb_oof_prob"]
    df_pool["_mean_oof_prob"] = df_scored["_mean_oof_prob"]
    df_pool["_consensus"] = (
        (df_pool["_rf_oof_prob"]  >= min_prob) &
        (df_pool["_xgb_oof_prob"] >= min_prob)
    )

    n_rep  = int(df_pool["_is_rep"].sum())
    n_cons = int(df_pool["_consensus"].sum())
    n_both = int((df_pool["_is_rep"] & df_pool["_consensus"]).sum())
    print("")
    print("[combined] Pool summary:")
    print("  Cluster representatives :", f"{n_rep:,}", "/", f"{len(df_pool):,}")
    print("  OOF consensus (>=" + str(min_prob) + ")   :", f"{n_cons:,}", "/", f"{len(df_pool):,}")
    print("  Both (Tier 1)           :", f"{n_both:,}", "/", f"{len(df_pool):,}")

    prefix = ("combined (cluster_col=" + cluster_col + ", thr=" + str(threshold)
              + ", kmer_col=" + kmer_col + ", min_prob=" + str(min_prob) + ", cv=" + str(cv) + ")")

    print("")
    print("[combined] Selecting balanced  n=" + f"{n_balanced:,}")
    sel_bal, note_bal = _combined_select_n(df_pool, n_balanced,   "balanced")

    print("")
    print("[combined] Selecting imbalanced n=" + f"{n_imbalanced:,}")
    sel_imb, note_imb = _combined_select_n(df_pool, n_imbalanced, "imbalanced")

    return sel_bal, sel_imb, prefix + " | balanced: " + note_bal + " | imbalanced: " + note_imb

def build_balanced_dataset(
    input_path, label_col,
    strategy="cluster", cluster_col="CDR3", threshold=0.8,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--min-prob",    default=0.6, type=float, dest="min_prob",
                        help="[kmer_consensus] OOF consensus threshold (default: 0.6)")
    ...
    build_balanced_dataset(
        ...
        cv=args.cv,               bal_path=args.bal_path,
        imb_path=args.imb_path,   reject_path=args.rejected,
