...
def load_ipi_elisa_dataset(main_path: str, data_file: str = DATA_FILE) -> pd.DataFrame:
    """
    Load IPI PSR full training set (ELISA + NGS combined).
    Returns all antibodies with a valid psr_filter label (n = 11,267).

    No ELISA-specific denoising filters are applied — the full training set
    is used to reflect the complete sequence diversity seen during model
    training, including high-polyreactivity Fail antibodies identified by
    NGS-based ssDNA selection.
    """
    data = pd.read_excel(os.path.join(main_path, data_file))
    data = data[pd.notna(data["psr_filter"])]
    print(f"  Loaded {len(data):,} antibodies (IPI PSR full trainset: ELISA + NGS)")
    return data
...
def compute_tsne(
    ...
    Default parameters match the original Delphi publication figure:
...
def generate_figure3(
    ...
    Dataset: IPI PSR full trainset (ELISA + NGS, n = 11,267)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Figure 3 t-SNE — 5 PLMs incl. IgBert "
    )
    ...
    parser.add_argument("--vh_col",        type=str,   default="vh_scaffold")
    parser.add_argument("--vl_col",        type=str,   default="vl_scaffold")
