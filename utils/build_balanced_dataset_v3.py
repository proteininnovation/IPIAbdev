...
def _combined_select_n(df_pool, n_target, floor_prob=0.5, label_str=""):
    """
    Round-robin selection across CDR3 clusters, ranked within each
    cluster by descending mean OOF probability.

    Within-cluster quality filter (applied before round-robin):
        A member is "good" if BOTH RF_oof >= floor_prob
                              AND XGB_oof >= floor_prob.
        This is a true dual-model consensus: one model agreeing is not
        enough. A high mean can hide one model strongly disagreeing
        (e.g. RF=0.9, XGB=0.1 → mean=0.5 but XGB predicts FAIL).

    Per-cluster fallback:
        If a cluster has NO "good" members, its single best member by
        mean OOF prob is retained as a fallback so that no cluster is
        ever silenced. Diversity is always preserved.

    Round-robin:
        Round 1: pick best "good" member from every cluster
        Round 2: pick 2nd-best "good" member from every cluster
        ...until n_target reached or all good members exhausted,
        then fall through to fallback members if still needed.

    floor_prob guideline:
        0.5  -- both models must predict PASS > FAIL (safe default)
        0.6  -- stricter; both models must be reasonably confident
    """
    tag = "[combined" + (" " + label_str if label_str else "") + "]"
    df  = df_pool.copy().reset_index(drop=True)

    # ── Dual-model consensus flag per row ─────────────────────────────────────
    # "good" = BOTH RF and XGB >= floor_prob (not just their mean)
    df["_good"] = (
        (df["_rf_oof_prob"]  >= floor_prob) &
        (df["_xgb_oof_prob"] >= floor_prob)
    )

    n_good  = int(df["_good"].sum())
    n_bad   = len(df) - n_good

    # Per-cluster: how many clusters have at least one good member?
    cluster_has_good   = df.groupby("_cluster")["_good"].any()
    n_clusters_good    = int(cluster_has_good.sum())
    n_clusters_nogood  = int((~cluster_has_good).sum())
    n_clusters_total   = df["_cluster"].nunique()

    print("  " + tag + " floor_prob=" + str(floor_prob)
          + "  (both RF_oof AND XGB_oof >= floor)")
    print("  " + tag + " good members      : " + f"{n_good:,}"
          + " / " + f"{len(df):,}"
          + "  (" + f"{n_good/len(df):.1%}" + ")")
    print("  " + tag + " excluded per-row  : " + f"{n_bad:,}"
          + "  (at least one model < floor)")
    print("  " + tag + " clusters w/ good  : " + f"{n_clusters_good:,}"
          + " / " + f"{n_clusters_total:,}")
    if n_clusters_nogood > 0:
        print("  " + tag + " clusters fallback : " + f"{n_clusters_nogood:,}"
              + "  (all members excluded → best member kept for diversity)")
    print("  " + tag + " target            : " + f"{n_target:,}")

    # ── Build effective pool: good members + per-cluster fallback ─────────────
    # Fallback: for clusters with no good members, keep the single highest-mean
    # member so diversity is never sacrificed
    good_pool    = df[df["_good"]].copy()

    if n_clusters_nogood > 0:
        nogood_clusters = cluster_has_good[~cluster_has_good].index
        fallback_pool   = (
            df[df["_cluster"].isin(nogood_clusters)]
            .sort_values("_mean_oof_prob", ascending=False)
            .groupby("_cluster")
            .first()
            .reset_index(drop=True)
        )
        effective_pool = pd.concat([good_pool, fallback_pool], ignore_index=True)
    else:
        effective_pool = good_pool.copy()

    # Drop any rows with NaN in _mean_oof_prob before ranking
    # (can occur after concat if a cluster had no scored members)
    effective_pool = effective_pool.dropna(subset=["_mean_oof_prob"]).copy()

    # ── Within-cluster rank on effective pool ─────────────────────────────────
    ...
    n_available = len(effective_pool)

    print("  " + tag + " effective pool    : " + f"{n_available:,}"
          + "  (good + fallbacks)")
    print("  " + tag + " max cluster depth : " + f"{max_round}")
    ...
    selected = effective_pool.loc[selected_idx]

    max_round_used  = int(selected["_within_rank"].max()) if len(selected) else 0
    n_round1        = int((selected["_within_rank"] == 1).sum())
    n_selected_good = int(selected["_good"].sum()) if "_good" in selected.columns else len(selected)
    n_selected_fall = len(selected) - n_selected_good
    mean_p          = float(selected["_mean_oof_prob"].mean())
    min_p           = float(selected["_mean_oof_prob"].min())

    print("  " + tag + " rounds used       : 1->" + str(max_round_used)
          + "  (round-1 = " + str(n_round1) + "/" + str(n_clusters_total) + " clusters)")
    print("  " + tag + " selected          : " + f"{len(selected):,}"
          + "  (good=" + str(n_selected_good)
          + "  fallback=" + str(n_selected_fall) + ")")
    print("  " + tag + " OOF prob  mean="  + f"{mean_p:.3f}"
          + "  min=" + f"{min_p:.3f}")

    if len(selected) < n_target:
        print("  [WARN " + tag + "] Only " + str(len(selected))
              + " rows in effective pool (< target " + str(n_target) + ")")
        print("  [WARN " + tag + "] Lower --floor-prob (currently "
              + str(floor_prob) + ") to include more candidates.")

    note = ("round-robin: " + str(n_clusters_total) + " clusters, "
            + "floor=" + str(floor_prob) + " (dual RF+XGB), "
    ...
    drop_cols = ["_cluster", "_rf_oof_prob", "_xgb_oof_prob",
                 "_mean_oof_prob", "_within_rank", "_good"]
    return selected.drop(columns=drop_cols, errors="ignore").reset_index(drop=True), note
